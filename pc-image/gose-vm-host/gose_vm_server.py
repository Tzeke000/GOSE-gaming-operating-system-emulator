...
# ===================== GPU + PLATFORM + STRESS (#101) — EXTRACTED MODULE =====================
# These three route groups (GPU-capability probe, platform/run-mode detection, and the
# CPU/GPU stress test) were moved VERBATIM into routes_stress_gpu.py on 2026-06-09 as the
# first step of splitting this monolith into per-domain modules.  They form a near-closed
# cluster (own private state + shared logger only), so isolating them stops an unrelated
# edit from silently clobbering the #101 stress endpoints again.  The names below are
# re-exported into this module so the do_GET/do_POST dispatch is byte-for-byte unchanged.
from routes_stress_gpu import (
    gpu_capability, gpu_capability_refresh, platform_detect,
    stress_start, stress_stop, stress_status,
)
